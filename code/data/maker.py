...
face_dir = '/home/dataset/FDDB/face_pos/'
                ...
                for x_ix, x_i in enumerate(x_n):
                    for y_idx, y_i in enumerate(y_n):
                        ...
                        if face_img.shape[0] == 0 or face_img.shape[1] == 0:
                            continue
                        cls = x_ix * len(y_n) + s_idx * len(y_n) * len(x_n) + y_idx
                        if cls == 22:
                            plt.imsave(os.path.join(face_dir,
                                                    str(index) + '-' + uid + '_{}' + '_{}.jpg').format(1, cls),
                                       face_img)
...
def make_data(pos=True, neg=True):
    if pos:
        make_pos_data(size=300000)
    if neg:
        make_neg_data(size=50000)


if __name__ == '__main__':
    make_data(pos=False, neg=True)
